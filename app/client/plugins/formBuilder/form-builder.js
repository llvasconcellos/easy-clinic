...
function formBuilderHelpersFn(opts, formBuilder) {
  ...
  _helpers.stickyControls = function ($sortableFields, cbUL) {
    ...
    var stageWrapOffset = $stageWrap.offset();

    $(window).scroll(function () {

      var scrollTop = $(this).scrollTop();

      if (scrollTop > stageWrapOffset.top) {
        var cbOffset = $cbWrap.offset(),
            cbBottom = cbOffset.top + $cbWrap.height(),
            stageBottom = stageWrapOffset.top + $stageWrap.height();

        if (cbBottom < stageBottom || cbBottom === stageBottom && cbOffset.top > scrollTop) {
          $cbWrap.css({
            position: 'absolute',
            top: scrollTop - cbPosition.height - (cbPosition.height / 2) + 10,
            right: 0
          });
        }
      } else {
